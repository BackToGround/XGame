...
[CustomEditor(typeof(SgtDisplacedMesh))]
public class SgtDisplacedMesh_Editor : SgtEditor<SgtDisplacedMesh>
{
	protected override void OnInspector()
	{
		...
		BeginError(Any(t => t.OriginalMesh == null));
			DrawDefault("OriginalMesh", ref rebuildMesh);
		EndError();
		BeginError(Any(t => t.HeightTex == null));
			DrawDefault("HeightTex", ref rebuildMesh);
		EndError();
		BeginError(Any(t => t.InnerRadius == t.OuterRadius));
			DrawDefault("InnerRadius", ref rebuildMesh);
			DrawDefault("OuterRadius", ref rebuildMesh);
		EndError();
